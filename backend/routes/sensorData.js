...
// Helper to format Date to YYYY-MM-DD HH:MM:SS in local (Malaysian) time
const formatToDeviceTimestamp = (date) => {
  ...
  try {
    ...
    const trendData = aggregatedData.map((point) => {
      const gasData = processAllGases({
        SN1_WE_V: point.SN1_WE_V,
        SN1_AE_V: point.SN1_AE_V,
        SN2_WE_V: point.SN2_WE_V,
        SN2_AE_V: point.SN2_AE_V,
        SN3_WE_V: point.SN3_WE_V,
        SN3_AE_V: point.SN3_AE_V,
        SN4_WE_V: point.SN4_WE_V,
        SN4_AE_V: point.SN4_AE_V,
      });
...
router.get('/history', async (req, res) => {
  try {
    const { startDate, endDate, deviceId, viewMode = 'auto', limit = 5000, page = 1 } = req.query;
    ...
    if (startDate || endDate) {
      ...
      if (startDate) {
        // Convert to timestamp_device format (YYYY-MM-DD HH:MM:SS) in local time
        const [y, m, d] = startDate.split('-').map(Number);
        const startDateTime = new Date(y, m - 1, d, 0, 0, 0);
        matchQuery['metadata.timestamp_device'].$gte = formatToDeviceTimestamp(startDateTime);
      }
      if (endDate) {
        const [y, m, d] = endDate.split('-').map(Number);
        const endDateTime = new Date(y, m - 1, d, 23, 59, 59);
        matchQuery['metadata.timestamp_device'].$lte = formatToDeviceTimestamp(endDateTime);
      }
    } else {
      const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
      matchQuery['metadata.timestamp_device'] = {
        $gte: formatToDeviceTimestamp(sevenDaysAgo),
      };
    }
    ...
    console.log(
      `📊 [${viewMode}] ${totalCount} points, ${daysDiff} days → ${shouldAggregate ? aggregationType + ' aggregation' : 'raw data'}`
    );
    ...
    if (!shouldAggregate) {
      ...
      return res.json({
        ...
        metadata: {
          ...
          viewMode: viewMode,
          dateRange: {
            start: formatToDeviceTimestamp(start),
            end: formatToDeviceTimestamp(end),
            days: daysDiff,
          },
        },
      });
    ...
    const pipeline = [
      { $match: matchQuery },
      {
        $addFields: {
          ...
          timestamp_date_part: {
            $substr: ['$metadata.timestamp_device', 0, aggregationType === 'daily' ? 10 : 13],
          },
        },
      },
      {
...
router.get('/history/export', async (req, res) => {
  try {
    ...
    if (format === 'csv') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="air-quality-export-${Date.now()}.csv"`
      );
      res.send(csv);
    } else {
      res.json({
        ...
        metadata: {
          ...
          exportedAt: formatToDeviceTimestamp(new Date()),
        },
      });
    }
  } catch (err) {
